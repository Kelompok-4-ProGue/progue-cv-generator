<template>
  <div class='sticky top-0 backdrop-blur-md bg-black w-full z-10 text-white flex items-center justify-center'>
        <!-- {/* Desktop App Bar */} -->
        <div class='max-w-ds w-full'>
          <div class='flex justify-between items-center py-5 md:justify-start md:space-x-10 h-14'>
            ...
            <div class='-mr-2 -my-2 md:hidden'>
              <button class='bg-white rounded-md p-2 inline-flex items-center justify-center text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500'>
            ...
            <div class='hidden md:flex space-x-10'>
                <div v-for="(menu,index) in menus" :key="index">
                    <router-link :to="menu.href" class="text-tl-sm font-medium hover:font-bold transition"  active-class="font-bold">{{menu.label}}</router-link>
                </div>
            </div>
            <div class='hidden md:flex items-center justify-end md:flex-1 lg:w-0'>
              <router-link to='/auth/login'>
                <button class='ml-5 whitespace-nowrap inline-flex items-center justify-center px-5 py-[6px] w-[86px] rounded-md shadow-sm text-lb-sm font-bold text-white border-[1px] border-blue bg-blue hover:bg-blue-700 cursor-pointer'>
                  Login
                </button>
              </router-link>
              <router-link to='/auth/register'>
                <div class='ml-5 whitespace-nowrap inline-flex items-center justify-center px-5 py-[6px] w-[86px] rounded-md shadow-sm text-lb-sm font-bold text-white border-[1px]  border-blue hover:bg-blue-700 cursor-pointer'>
                  Register
                </div>
              </router-link>
            </div>
        ...
        <!-- <Transition as={Fragment} enter='duration-200 ease-out' enterFrom='opacity-0 scale-95' enterTo='opacity-100 scale-100' leave='duration-100 ease-in' leaveFrom='opacity-100 scale-100' leaveTo='opacity-0 scale-95'>
          <Popover.Panel focus class='absolute top-0 inset-x-0 p-2 transition transform origin-top-right md:hidden'>
            <div class='rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 bg-white divide-y-2 divide-gray-50'>
              <div class='pt-5 pb-6 px-5'>
                <div class='flex items-center justify-between'>
                  <div>
                    <h1 class='text-3xl font-bold'>
                      <span class='box-decoration-slice bg-gradient-to-r from-blue-600 to-pink-500 text-white py-1 px-3 rounded-md' href='https://nextjs.org'>
                        Musically
                      </span>
                    </h1>
                  </div>
                  <div class='-mr-2'>
                    <Popover.Button class='bg-white rounded-md p-2 inline-flex items-center justify-center text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500'>
              ...
              <div class='py-6 px-5 space-y-6'>
                <div class='grid grid-cols-2 gap-y-4 gap-x-8'>
                  {menus.map((menu, index) => {
                    return (
                      <div key={index}>
                        <Link href={menu.href}>
                          <a class={`${menu.label === active ? 'text-gray-900' : 'text-gray-500'} text-base font-medium text-gray-900 hover:text-gray-700`}>{menu.label}</a>
                        </Link>
                      </div>
                    );
                  })}
                ...
                <div>
                  <p class='mt-6 text-center text-base text-gray-500'>
                    Wanna leave?{' '}
                    <button onClick={() => {}} href='#' class='text-blue-600 hover:text-blue-500 font-bold'>
                      Sign Out
                    </button>
                  </p>
                </div>
